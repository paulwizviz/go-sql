...
#### Querying with `pgx`
 ...
 In the pgx library, the choice of method depends on whether you expect data back from the database:

- `pool.Exec` (Side Effects Only), Use this for INSERT, UPDATE, DELETE, or CREATE when you do not need to get any data back (like an ID).
  - Example: pool.Exec(ctx, "DELETE FROM users")

- `pool.QueryRow` (One Row Back). Use this when your INSERT has a RETURNING clause and you want to capture a single value (like the autogenerated uuidv7). This is what I used in your code for the person and collective inserts.
  - In your code: `pool.QueryRow(ctx, insertPersonSQL).Scan(&u)`

- `pool.Query` (Multiple Rows Back). This is indeed typically for SELECT operations where you expect a result set to iterate over. While you could use it for an `INSERT ... RETURNING` that returns multiple rows, it is much more common for queries.
  - In your code: pool.Query(ctx, selectMembersSQL, collectiveUUID)

### Comparison between `pgx` and `lib/pq`
...
### Example 4: Using UUID type as Primary Key

This [example](../examples/coding/pg/ex4/main.go) demonstrates the use of UUID type as PRIMARY KEY and manipulating relational operations. The package `pgx` is used.
