...
- **SQLite:** Yes (Full Transactional DDL)
  ...
  - **Exceptions**: Some DDL operations are not supported at all in SQLite, which limits what you can do inside a transaction:
    - `DROP COLUMN` was only added in SQLite 3.35.0 (2021) — older versions don't support it at all.
    - `RENAME COLUMN` was only added in SQLite 3.25.0 (2018).
    - Complex `ALTER TABLE` operations like changing a column's type or constraints are not supported — the common workaround is to create a new table, copy data, drop the old table, and rename the new one, all within a transaction.
  - **Benefit**: Because SQLite is file-based and single-process, transactional DDL is particularly valuable for safe, atomic schema migrations without a running server. A failed migration leaves the file completely untouched, making it easy to recover — just fix the script and re-run. This makes SQLite especially well suited for embedded applications and CLI tools that need to manage their own schema evolution reliably.
